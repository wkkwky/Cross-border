                ...
                @foreach($package_payment_requests as $key => $package_payment_request)
                    @if ($package_payment_request->user != null)
                        <tr>
                            ...
                            <td>{{ $package_payment_request->seller_spread_package->name }}</td>
...
@section('script')
    <script type="text/javascript">
        function offline_payment_approval(el){
            ...
            $.post('{{ route('offline_seller_spread_package_payment.approved') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
