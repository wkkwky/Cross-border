...
<div class="aiz-titlebar text-left mt-2 mb-3 row align-items-center">
    ...
    <!--<div class="col text-right">
        <a href="{{ route('salesman.customers.create') }}" class="btn btn-circle btn-info">
                    ...
                    @foreach($users as $key => $user)
                        @if ($user != null)
                            <tr>
                                ...
                                <td>@if($user->banned == 1) <i class="fa fa-ban text-danger" aria-hidden="true"></i> @endif {{$user->name}} @if($user->is_virtual_user == 1) (<font color="red">{{translate('Virtual')}}</font>) @endif</td>
                                <td>{{$user->email}}</td>
                                <td>{{$user->phone}}</td>
                                <td>
                                ...
                                <td class="text-right">
                                    ...
                                    @else
                                    <a href="#" class="btn btn-soft-success btn-icon btn-circle btn-sm" onclick="confirm_unban('{{route('salesman.customers.ban', encrypt($user->id))}}');" title="{{ translate('Unban this Customer') }}">
                                        <i class="las la-user-check"></i>
                                    </a>
                                    @endif
                                    <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('salesman.customers.destroy', $user->id)}}" title="{{ translate('Delete') }}">
