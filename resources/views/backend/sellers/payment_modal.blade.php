<form action="{{ route('commissions.pay_to_seller') }}" method="POST">
    ...
    <div class="modal-body">
      <table class="table table-striped table-bordered" >
          <tbody>
              <tr>
                  @if($shop->admin_to_pay >= 0 || 1 )
                      <td>{{ translate('Pending Balance') }}</td>
      ...
      @if ($shop->admin_to_pay > 0 && 0 )
          <div class="form-group row">
              <label class="col-md-3 col-from-label" for="amount">{{translate('Operator')}}</label>
              <div class="col-md-9">
                  <input type="text" lang="en"   name="amount" id="amount" value="" class="form-control" required>
              </div>
          </div>
    <!--
          <div class="form-group row">
      ...
      @else
          <div class="form-group row">
              <label class="col-md-3 col-from-label" for="amount">{{translate('Operator')}}</label>
              <div class="col-md-9">
                  <input type="text" lang="en" name="amount" id="amount"   class="form-control" required>
              </div>
          </div>
          <div class="form-group row" id="txn_div">
              <label class="col-md-3 col-from-label" for="txn_code">{{translate('Txn Code')}}</label>
